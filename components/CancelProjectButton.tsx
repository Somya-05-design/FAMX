...
export function CancelProjectButton({ projectId, status }: CancelProjectButtonProps) {
  ...
  return (
    <button
      ...
      disabled={isPending}
      className="px-4 py-2 bg-error/10 hover:bg-error/20 border border-error/30 text-error font-bold rounded-xl text-xs shadow-xs transition-all disabled:opacity-50 cursor-pointer flex items-center space-x-1.5"
    >
      {isPending ? (
