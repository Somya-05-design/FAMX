...
export function KanbanBoard({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0 pb-4 border-b border-zinc-200/60">
        <div>
        ...
        {/* Mock user avatar stack + Assignee button per Section 4 */}
        <div className="flex items-center space-x-3">
          <div className="flex -space-x-2.5 overflow-hidden">
            <div className="w-8 h-8 rounded-full border-2 border-white bg-gradient-to-tr from-amber-400 to-rose-400 flex items-center justify-center text-white text-[9px] font-black shadow-sm">
              JD
            </div>
            <div className="w-8 h-8 rounded-full border-2 border-white bg-gradient-to-tr from-purple-400 to-indigo-400 flex items-center justify-center text-white text-[9px] font-black shadow-sm">
              ST
            </div>
            <div className="w-8 h-8 rounded-full border-2 border-white bg-gradient-to-tr from-emerald-400 to-teal-400 flex items-center justify-center text-white text-[9px] font-black shadow-sm">
              AL
            </div>
            <div className="w-8 h-8 rounded-full border-2 border-white bg-zinc-100 flex items-center justify-center text-zinc-500 text-[9px] font-bold shadow-sm select-none">
              +8
            </div>
          </div>
          <button className="flex items-center space-x-1.5 px-3 py-1.5 bg-white border border-zinc-200 text-zinc-500 hover:text-zinc-700 hover:bg-zinc-50 rounded-xl text-xs font-semibold shadow-sm transition-all cursor-not-allowed opacity-60">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
          ...
          <div className="relative flex-1 sm:w-64 md:w-80 min-w-[200px]">
            ...
            <input
              ...
              className="w-full bg-white border border-zinc-200 focus:border-orange-500 rounded-xl pl-10 pr-4 py-2.5 text-xs text-zinc-700 outline-none transition-all shadow-sm"
            />
          </div>

          {/* Filter Dropdown Placeholder */}
          <button className="flex items-center space-x-1.5 px-4 py-2.5 bg-white border border-zinc-200 text-zinc-500 hover:text-zinc-700 hover:bg-zinc-50 rounded-xl text-xs font-semibold shadow-sm transition-all cursor-not-allowed opacity-60">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all border shadow-sm cursor-pointer ${showCancelled
                ? "bg-orange-600 border-transparent text-white hover:bg-orange-500 shadow-orange-950/20"
                : "bg-black border-transparent text-white hover:bg-zinc-800"
              }`}
          >
      ...
      {viewParam === "notifications" ? (
        ...
        <div className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm space-y-6 animate-fadeIn">
          <div className="flex justify-between items-center pb-4 border-b border-zinc-200/60">
            <h2 className="text-lg font-black text-zinc-900 tracking-tight">System Activity Alerts</h2>
            {notifications.some((n) => !n.read) && (
              <button
                onClick={handleMarkAllAsRead}
                className="px-4 py-2 bg-black hover:bg-zinc-800 text-white text-xs font-bold rounded-xl shadow-sm transition-all"
              >
                Mark all as read
              </button>
            )}
          ...
          <div className="space-y-4">
            {notifications.length === 0 ? (
              <div className="py-16 text-center text-zinc-400 select-none">
                ...
                <p className="text-xs text-zinc-400 mt-1">You have no system activity notifications.</p>
              </div>
            ) : (
              notifications.map((notification) => (
                <div
                  key={notification.id}
                  className={`flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-4 border rounded-2xl transition-all ${!notification.read
                ...
                >
                  <div className="flex items-start space-x-3.5 min-w-0">
                    {/* Glowing status dot */}
                    <span
                      className={`w-2.5 h-2.5 rounded-full mt-1.5 shrink-0 ${!notification.read
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-zinc-500 mt-1 leading-relaxed truncate max-w-[280px] sm:max-w-md md:max-w-xl xl:max-w-4xl">
                  ...
                  <div className="flex items-center space-x-2 shrink-0 self-end sm:self-center">
                    {notification.projectId && (
                      <Link
                        ...
                        className="px-3.5 py-1.5 border border-zinc-200 hover:bg-zinc-50 text-zinc-600 rounded-xl text-xs font-bold transition-all shadow-sm"
                      >
                        View Workspace
                      </Link>
                    )}
                    ...
                    {!notification.read && (
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-orange-600 hover:bg-orange-500 text-white rounded-xl text-xs font-bold transition-all shadow-sm"
                      >
                        Mark Read
                      </button>
                    )}
        ...
        /* Cancelled View Table (styled visually with white background cards/tables) */
                ...
                {cancelledProjects.length === 0 ? (
                  ...
                  cancelledProjects.map((project) => (
                    <tr key={project.id} className="hover:bg-zinc-50/50 transition-colors">
                      <td className="px-6 py-4">
                        <div>
                          <Link
                            ...
                            className="font-bold text-zinc-800 hover:text-orange-600 transition-colors text-sm"
                          >
                            {project.title}
                          </Link>
                      ...
                      <td className="px-6 py-4">
                        <div>
                          <p className="font-semibold text-zinc-700">{project.client.name || "Client"}</p>
                          <p className="text-[10px] text-zinc-400 mt-0.5">{project.client.email}</p>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link
                          href={`/admin/projects/${project.id}`}
                          className="inline-flex items-center space-x-1 text-xs font-bold text-orange-600 hover:text-orange-500 transition-colors"
                        >
                          <span>Manage</span>
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
